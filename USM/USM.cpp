...
USM::USM(const std::string &name)
    ...
    if (InTextMode) {
        ...
        if (is_opened) {
                            ...
                            if (!(sl->to_string(1, 5) == "<\\e>")) {
                                ...
                                switch (type) {
                                    case 0:
                                        ...
                                        break;
                                    case 1:
                                        ivec.emplace_back(std::stol(obj_buff));
                                        break;
                                }
                                obj_buff.clear();
                                sl = sl->end();
            ...
            system(((std::string(MAKEDIR) + "profiles" + name_)).c_str());
            system((std::string(CREATE_FILE) + "profiles/" + name_ + ".uto").c_str());
            system((std::string(CREATE_FILE) + "profiles/profiles_list.txt").c_str());
...
std::vector<USM> USM::get_profiles(const std::string& program_name) {
    std::vector<USM> profiles;
...
std::vector<USM> USM::get_profiles() {
    std::vector<USM> profiles;
    std::string path("profiles" + std::string(SEPARATOR) + "profiles_list.txt");
    std::ifstream file(path);
    if (file) {
...
IntSection& USM::geti (const std::string& name) {
    return isecs_.at(name);
}

StringSection& USM::gets (const std::string& name) {
    return ssecs_.at(name);
}


void USM::create_isec(const std::string &sec_name) {
    isecs_.emplace(sec_name, IntSection(sec_name));
}

void USM::create_ssec(const std::string &sec_name) {
    ssecs_.emplace(sec_name, StringSection(sec_name));
}

const bool USM::opened() {
    return is_opened;
}
